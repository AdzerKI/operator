...
func (cr *VTCluster) PrefixedName(kind vmv1beta1.ClusterComponent) string {
	if cr.Spec.UseLegacyNaming {
		return vmv1beta1.ClusterSuffixedName(kind, cr.Name, "vt", false)
	}
...
func (cr *VTCluster) PrefixedInternalName(kind vmv1beta1.ClusterComponent) string {
	if cr.Spec.UseLegacyNaming {
		return vmv1beta1.ClusterSuffixedName(kind, cr.Name, "vt", true)
	}
...
func (cr *VTInsert) GetMetricsPath() string {
	if cr == nil {
		return healthPath
	}
...
// GetMetricsPath returns prefixed path for metric requests
func (cr *VTSelect) GetMetricsPath() string {
	if cr == nil {
		return healthPath
	}
	return vmv1beta1.BuildPathWithPrefixFlag(cr.ExtraArgs, metricsPath)
...
func (cr *VTCluster) Validate() error {
	...
	if cr.Spec.Select != nil {
		...
		if nodes, ok := cr.Spec.Select.ExtraArgs["storageNode"]; ok {
		...
		for _, node := range cr.Spec.Select.ExtraStorageNodes {
