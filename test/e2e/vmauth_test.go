		...
		Context("crud", func() {
			...
			DescribeTable("should create vmauth", func(name string, cr *vmv1beta1.VMAuth, verify func(cr *vmv1beta1.VMAuth)) {
				...
				Entry("with 1 replica", "replica-1", &vmv1beta1.VMAuth{
					...
					Expect(expectPodCount(ctx, k8sClient, &appsv1.ReplicaSet{
						ObjectMeta: metav1.ObjectMeta{
							Namespace: namespace,
 ...
 unauthorized_user:
  ...
  - src_paths:
    ...
    url_prefix: "http://vmsingle-some-url:8428"`,
		...
		It("should skip reconciliation when VMAuth is paused", func() {
			ctx := context.Background()
			nsn.Name = "vmauth-paused"
			By("creating a VMAuth")
			...
			By("attempting to scale the VMAuth while paused")
